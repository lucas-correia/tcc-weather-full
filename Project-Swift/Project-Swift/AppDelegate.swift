...
@UIApplicationMain
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate, SideMenuDelegate, MessagingDelegate {
    ...
    func messaging(_ messaging: Messaging, didReceiveRegistrationToken fcmToken: String) {
        ...
        if(ud.value(forKey: App.Constants.PLISTKEY_PUSH_NOTIFICATION_TOKEN) == nil) {
            ...
            App.Connection.registerTokenWithParameters(params: params) { (response, statusCode, error) in
                
                if(statusCode == 200) {
                    
                    ud.set(fcmToken, forKey: App.Constants.PLISTKEY_PUSH_NOTIFICATION_TOKEN)
                    ud.synchronize()
                    print("token registrado")
                }
            }
        } else if(ud.value(forKey: App.Constants.PLISTKEY_PUSH_NOTIFICATION_TOKEN) as! String != fcmToken) {
...
//
//        let ud:UserDefaults = UserDefaults.standard
//        ud.set(token , forKey: App.Constants.PLISTKEY_PUSH_NOTIFICATION_TOKEN)
//        ud.synchronize()
        
        //InstanceID.instanceID().set setAPNSToken(deviceToken, type:.unknown)
